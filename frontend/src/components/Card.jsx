...
function CardComponent({game, open, onclick, close, pop, onDragStart, statusGame}) {

  
  return (
    <div className='flex flex-wrap gap-5'>
        <Card className="max-w-sm mt-3 cursor-grab active:opacity-[0.5] active:text-gray-100" data-id={game._id} draggable={true} onDragStart={(event) => onDragStart(event)}>
        ...
        <p className="font-normal text-gray-700 dark:text-gray-400">state: {`${statusGame}` ? `${statusGame}`: `${game.state}`}</p>
        { pop &&
